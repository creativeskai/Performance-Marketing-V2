...
function renderLiveInsights() {
  ...
  if (dailyAll.length > 0 && paEl) {
    ...
    const days = Object.entries(byDate).map(([date, d]) => ({ date, ...d, roas: d.spend > 0 ? d.revenue / d.spend : 0 })).sort((a, b) => { const months={'Jan':1,'Feb':2,'Mar':3,'Apr':4,'May':5,'Jun':6,'Jul':7,'Aug':8,'Sep':9,'Oct':10,'Nov':11,'Dec':12}; const pa=a.date.split(' '),pb=b.date.split(' '); return (months[pa[0]]*100+parseInt(pa[1]))-(months[pb[0]]*100+parseInt(pb[1])); });
